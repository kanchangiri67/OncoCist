...
def get_recent_user_scans(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    Returns:
        dict: A dictionary containing the user's details and recent scan history.
    """
    scans = (
        db.query(Scan)
...
def get_all_user_scans(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    Returns:
        dict: A dictionary containing the user's details and full scan history.
    """
    scans = (
        db.query(Scan)
        .filter(Scan.user_id == current_user.id)
...
def get_patients_of_user(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    Returns:
        list: A list of patients linked to the user's scans.
    """
    patient_ids = (
    ...
    patient_ids = [p[0] for p in patient_ids]  # Extract IDs from tuples
...
def get_recent_patient_scans(
    ...
    Returns:
        dict: A dictionary containing the patient's details and recent scan history.
...
def get_all_patient_scans(
    ...
    Returns:
        dict: A dictionary containing the patient's details and full scan history.
...
def delete_scan(scan_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    Raises:
        HTTPException: If the scan does not exist or the user is unauthorized.
    """
